...
class ConvBlock(nn.Module):
    def __init__(self, input_channels, output_channels, max_pool_size, ):
...
class VGG16(nn.Module):
    def __init__(self, num_classes, input_size=32):
        ...
        for layer_id, channel in enumerate(self.conv_channels):
            ...
            self.layers.append(ConvBlock(input_channel, channel, self.max_pool_sizes[layer_id]))
    ...
    def forward(self, x, with_latent=False):
        ...
        for layer in self.layers:
            if j == 13:
                out_latent = layer.layers[0](fwd)
                #print(layer.layers[0])
            fwd = layer(fwd)
            j = j+1
        fwd = self.end_layers(fwd)
        if with_latent:
            return fwd, out_latent
        return fwd
